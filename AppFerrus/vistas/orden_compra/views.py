...
class orden_compralistview(ListView):
    ...
    @method_decorator(permission_required('AppFerrus.view_ordencompramaterial'))
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'searchdata':
                ...
                data = []
                for i in DetOrdenCompra.objects.filter(orden_compra_material_id=request.POST['id']):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['create_url'] = reverse_lazy('ordencompracrear')
        context['list_url'] = reverse_lazy('ordencompralistado')
        return context
...
class orden_compraCreateView(CreateView):
    ...
    success_url = reverse_lazy('ordencompralistado') #direccion hacia donde voy a redireccionar
    
    #aqui viy a definir para cuando se aguarde un dato repetido
    @method_decorator(permission_required('AppFerrus.add_ordencompramaterial')) 
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'search_articulo': #la variable de mi form js
                ...
                for i in products:
                    det = DetOrdenCompra()
                    det.orden_compra_material_id = ordencompra.id
...
class ordencompraUpdateView(UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'search_articulo': #la variable de mi form js
                ...
                data= []
                for i in DetOrdenCompra.objects.filter(ordencompramaterial_id=request.POST['id']):
                    data.append(i.toJSON())
    ...
    def get_articulo_detalles(self): #para llamar mis productos
        data = [] #lo hago diccionario
        try:
            for i in DetOrdenCompra.objects.filter(ordencompramaterial_id=self.get_object().id):
                item = i.articulo.toJSON() #aqui pido mis articulos
                item['cant'] = i.cant #aqui pido mi cantidad
                data.append(item) #aqui llamo al diccionario
        except: 
